...
const NAV = [
  ...
  { label:"Performance", icon:TrendingUp, href:"/dashboard/performance" },h
  { section:"NETWORK" },
    ...
    <div className="flex h-screen bg-[#0A0A0A] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        <header className="sticky top-0 z-10 bg-[#0A0A0A] border-b border-[#2A2A2A] px-4 sm:px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-4"><button className="md:hidden text-gray-400 hover:text-white" onClick={()=>setOpen(true)}><Menu size={20}/></button><div><p className="text-xs text-gray-500">Finance · Recoveries</p><h1 className="font-serif text-lg font-bold text-white">Collections</h1></div></div>
          <div className="flex items-center gap-3"><button className="flex items-center gap-1.5 border border-[#2A2A2A] hover:border-white text-gray-400 hover:text-white text-xs px-3 py-1.5 rounded-lg transition-colors"><Download size={13}/>Export</button><button className="relative text-gray-400 hover:text-white"><Bell size={18}/><span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-[#C41E3A] rounded-full text-[8px] flex items-center justify-center font-bold text-white">4</span></button><a href="https://tutacars.co.za" target="_blank" rel="noopener" className="text-gray-500 hover:text-white"><ExternalLink size={16}/></a></div>
        </header>
        <main className="flex-1 px-4 sm:px-6 py-6 space-y-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[{label:"Total Overdue",value:"R112,500",sub:"4 accounts",icon:AlertTriangle,color:"text-[#C41E3A]"},{label:"Critical / Escalated",value:"1",sub:"Surety draw pending",icon:FileText,color:"text-red-400"},{label:"Promise to Pay",value:"1",sub:"Due 12 Jun",icon:Phone,color:"text-amber-400"},{label:"Cleared MTD",value:"R67,500",sub:"8 accounts resolved",icon:Mail,color:"text-emerald-400"}].map(k=>(
          ...
          <div className="bg-[#141414] border border-[#2A2A2A] rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-[#2A2A2A] flex items-center justify-between"><h2 className="font-serif text-sm font-bold text-white">Open Accounts</h2><span className="text-xs text-red-400 font-medium">{ACCOUNTS.length} accounts requiring action</span></div>
            <div className="divide-y divide-[#1E1E1E]">{ACCOUNTS.map(a=>(
              <div key={a.pod} className="p-4 hover:bg-[#1A1A1A]"><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><div className="flex items-center gap-2 mb-1"><span className="text-white font-medium text-sm">{a.name}</span><span className="text-gray-500 text-xs">· {a.pod}</span>{badge(a.priority)}</div><p className="text-[10px] text-gray-500 mb-2">{a.type} · {a.daysOverdue} days overdue · Contact: {a.contact}</p><p className="text-xs text-gray-400">Last action: <span className="text-gray-300">{a.action}</span> on {a.lastContact}</p></div><div className="flex items-center gap-4 flex-shrink-0"><div className="text-right"><p className="font-serif font-bold text-[#C41E3A] text-lg">{a.overdue}</p><p className="text-[10px] text-gray-500">overdue</p></div><div className="flex gap-2"><button className="flex items-center gap-1 text-gray-400 hover:text-white border border-[#2A2A2A] hover:border-[#444] text-xs px-2.5 py-1.5 rounded-lg transition-colors"><Phone size={11}/>Call</button><button className="flex items-center gap-1 text-gray-400 hover:text-white border border-[#2A2A2A] hover:border-[#444] text-xs px-2.5 py-1.5 rounded-lg transition-colors"><Mail size={11}/>Email</button></div></div></div></div>
            ))}</div>
          </div>
          <div className="bg-[#141414] border border-[#2A2A2A] rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-[#2A2A2A]"><h2 className="font-serif text-sm font-bold text-white">Collections Activity Log</h2></div>
            <div className="overflow-x-auto"><table className="w-full text-xs"><thead><tr className="border-b border-[#2A2A2A]">{["Date","Pod","Agent","Action","Outcome"].map(h=><th key={h} className="px-4 py-2.5 text-left text-gray-500 font-medium uppercase tracking-wider text-[10px]">{h}</th>)}</tr></thead>
            <tbody className="divide-y divide-[#1E1E1E]">{LOG.map((l,i)=>(<tr key={i} className="hover:bg-[#1A1A1A]"><td className="px-4 py-3 text-gray-400">{l.date}</td><td className="px-4 py-3 text-white font-medium">{l.pod}</td><td className="px-4 py-3 text-gray-400">{l.agent}</td><td className="px-4 py-3 text-gray-300">{l.action}</td><td className="px-4 py-3">{badge(l.outcome)}</td></tr>))}</tbody></table></div>
          </div>
        </main>
      </div>
    </div>
